<?php
    include "header.php"
?>



<div class="container " id="news" >
<div class="row">
<div class="col-md-8">
    <header class="page-header">
        <h1 class="page-title web-cht">分類：優惠精選</h1>
    </header>
    <article class="blog-post tl">
        <div class="row">
            <div class="col-sm-3 col-xs-4">

                <div class="featured-image">
                    <a href="news_l1.php"  class="blog-featured-img" >
                        <img src="images/news/n1.jpg" alt="">
                    </a>
                </div>
            </div>
            <div class="col-sm-7 col-xs-8 tl">
                <h3 class="main-heading "><a href="news_l1.php">【專程，創玩藝】手作體驗專案</a></h3>
                <div class="bolg-meta">
                    <span class="date">2015-06-20</span>
                    <span class="cate "><a href="news_c1.php">旅程限定</a></span>
                </div>
                <div class="summary ">
                    三天二夜 | 二天一夜 優惠<br/>

                    設計的實演基地，工藝的摩娑試驗；<br/>
                    誠品行旅，一個讓人躍躍欲試的地方。
                </div>
                <a class="more " href="news_l1.php">深入閱讀</a>
            </div>
        </div>
    </article>



    <article class="blog-post tl">
        <div class="row">
            <div class="col-sm-3 col-xs-4">

                <div class="featured-image">
                    <a href="news_l1.php"  class="blog-featured-img" >
                        <img src="images/news/n2.jpg" alt="">
                    </a>
                </div>
            </div>
            <div class="col-sm-7 col-xs-8 tl">
                <h3 class="main-heading "><a href="news_l1.php">整裝待發</a></h3>
                <div class="bolg-meta">
                    <span class="date ">2015 / 06 / 20</span>
                    <span class="cate "><a href="news_c2.php">優惠精選</a> , <a href="news_c1.php">旅程限定</a></span>
                </div>
                <div class="summary ">
                    【穿說，城內城外世】<br/>
                    歷史生態的爬梳探勘<br/>
                    藝文渠道的親臨拓寬
                </div>
                <a class="more " href="news_l1.php">深入閱讀</a>
            </div>
        </div>
    </article>



    <article class="blog-post tl">
        <div class="row">
            <div class="col-sm-3 col-xs-4">

                <div class="featured-image">
                    <a href="news_l1.php"  class="blog-featured-img" >
                        <img src="images/news/n3.jpg" alt="">
                    </a>
                </div>
            </div>
            <div class="col-sm-7 col-xs-8 tl">
                <h3 class="main-heading "><a href="news_l1.php">【In Between Steakhouse 信用卡優惠】</a></h3>
                <div class="bolg-meta">
                    <span class="date">2015-06-08</span>
                    <span class="cate "><a href="news_c3.php">美饌佳餚</a></span>
                </div>
                <div class="summary ">
                    持中信、富邦、花旗限定卡別信用卡用餐享五折起優惠<br/>
                    活動期間：104/07/17 至 2015/09/30
                </div>
                <a class="more web-cht" href="news_l1.php">深入閱讀</a>
            </div>
        </div>
    </article>


    <article class="blog-post tl">
        <div class="row">
            <div class="col-sm-3 col-xs-4">

                <div class="featured-image">
                    <a href="news_l1.php"  class="blog-featured-img" >
                        <img src="images/news/n4.jpg" alt="">
                    </a>
                </div>
            </div>
            <div class="col-sm-7 col-xs-8 tl">
                <h3 class="main-heading "><a href="news_l1.php">【Chef Table Menu】紀念的回甘款待</a></h3>
                <div class="bolg-meta">
                    <span class="date">2015-06-01</span>
                    <span class="cate "><a href="news_c3.php">美饌佳餚</a></span>
                </div>
                <div class="summary ">
                    花瓣簇擁，主廚精選，<br/>
                    瓊漿玉液，款待切切。<br/>
                    讓每段紀念日都念念不忘，<br/>
                    讓每一口都回甘生香。
                </div>
                <a class="more " href="news_l1.php">深入閱讀</a>
            </div>
        </div>
    </article>


    <article class="blog-post tl">
        <div class="row">
            <div class="col-sm-3 col-xs-4">

                <div class="featured-image">
                    <a href="news_l1.php"  class="blog-featured-img" >
                        <img src="images/news/n5.jpg" alt="">
                    </a>
                </div>
            </div>
            <div class="col-sm-7 col-xs-8 tl">
                <h3 class="main-heading "><a href="news_l1.php">【精選開胃沙拉吧】向晚的異國食尚</a></h3>
                <div class="bolg-meta">
                    <span class="date ">2015-05-30</span>
                    <span class="cate "><a href="news_c3.php">美饌佳餚</a></span>
                </div>
                <div class="summary ">
                    料理的多彩熱情、婉約細膩，<br/>
                    味蕾的層層探險、遠渡重洋。<br/>
                    向晚的夜色與異國風情相伴，<br/>
                    濃縮成馥郁的美好。
                </div>
                <a class="more " href="news_l1.php">深入閱讀</a>
            </div>
        </div>
    </article>


</div>
<div class="col-md-4  sidebar">


    <div id="search-2" class="widget widget_search side_block">
        <form role="search" method="get" id="searchform" class="searchform" action="#">
            <div>
                <label class="screen-reader-text" for="s">搜尋關於：</label>
                <input type="text" value="" name="s" id="s">
                <input type="submit " id="searchsubmit" value="搜尋">
            </div>
        </form>
    </div>

    <div class="clear"></div>



    <div id="recent-posts" class="widget tl ">
        <h4 class="sub-heading"><span class="highlight-txt">分類</span></h4>

        <ul>
            <li>
                <a href="#">最新消息</a>
            </li>
            <li>
                <a href="#">旅程限定</a>
            </li>
            <li>
                <a href="#">美饌佳餚</a>
            </li>
            <li>
                <a href="#">系列活動</a>
            </li>
            <li>
                <a href="#">展覽</a>
            </li>
            <li>
                <a href="#">園區活動</a>
            </li>
        </ul>
    </div>


    <div id="recent-posts" class="widget tl ">
        <h4 class="sub-heading"><span class="highlight-txt">近期訊息</span></h4>

        <ul>
            <li>
                <a href="#">生日貴賓專屬贈禮</a>
            </li>
            <li>
                <a href="#">週末滿額贈禮</a>
            </li>
            <li>
                <a href="#">【慢遊之旅】　　　三天二夜專案</a>
            </li>
            <li>
                <a href="#">【專程，創玩藝】　手作體驗專案</a>
            </li>
            <li>
                <a href="#">【穿說，城內外世】 國人暑期藝文專案</a>
            </li>
        </ul>
    </div>



    <div id="recent-posts" class="widget tl ">
        <h4 class="sub-heading"><span class="highlight-txt">影片</span></h4>
        <div>
            <iframe style="min-height:215px;width: 100%;" width="100%" height="auto" src="https://www.youtube.com/embed/rbyVJlSu1XI" frameborder="0" allowfullscreen></iframe>
        </div>
    </div>



</div>
</div>
</div>

<?php
include "footer.php"
?>
